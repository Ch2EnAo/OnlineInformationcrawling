...
class _Mail:
    ...
    def Send_mail(self):
        ...
        try:
            ...
            logging.log(logging.INFO, "提醒邮件发送成功")
        except smtplib.SMTPException:
            logging.log(logging.INFO, "提醒邮件发送成功,请检查账号是否开启SMTP服务，授权码是否正确")
    ...
    def Polling_Mail(self):
        ...
        if not self.LOGIN_STATE:    # 判断当前是否登录
            ...
            self.state = self.Login()    # 登录并返回登录状态
        try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            Start(account=params[0], password=params[1], polling_time=int(params[2]))
        except ValueError:
            logging.log(logging.INFO, "the code polling time is intager type")
        except IndexError:
            logging.log(logging.INFO,
                        """:param account: the mail account \n:param password: the mail password  \n:param polling_time: the code polling time"""
            )
